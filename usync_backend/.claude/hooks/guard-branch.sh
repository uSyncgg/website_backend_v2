...
#     owner-only, using the pr-to-main skill).
#   - staging_environment   → staging. Blocked for contributors; the owner is
...
if [ "$branch" = "staging_environment" ] && [ "$email" != "$owner_email" ]; then
  echo "Blocked: you are on the staging branch 'staging_environment'. Contributors don't edit staging directly. Create a feature branch first (ask: \"start a new branch called feature/<short-name>\"), then make changes — it will reach staging through a pull request." >&2
  exit 2
fi
